...
\abstract{
	...
	In this combination of models, one can leverage the physical proximity of an application with its users to enable real-time scnarios, reduce bandwidth usage, etc. ; however, multiple applications will require doing so, and efficiently.\\
	...
	This bibliographic report will concentrate on finding open working directions from the literature, paving the way toward a MSc. internship. We will pay attention to the metrics taken into account, acknowledging the algorithms as well as making a case for auctions. \\
	We think auction -- and bidding -- paired with a good contracting such as \glspl{SLA} would translate the needs of an application's functions and enable efficient exploitation of the Fog by correctly provisioning those functions. Furthermore, it would provide an an introduction in Fog's economics as well as considering \gls{FaaS} challenges (cold-start, function storage, input data) and make security and privacy a priority.
}
