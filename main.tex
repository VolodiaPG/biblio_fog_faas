...
%\end{itemize}
\cref{tab:placement} summarizes points of comparison for approaches detailed in subsection \cref{sec:literature_review}. Based on this table, we can conjecture a list of questions:
...
\begin{description}
	\item[Lack of contract based transactions] that would detail all challenges posed by a transition from Cloud to Fog, namely \cite{chiang_fog_2016, bonomi_fog_2012}:
		...
		\end{enumerate*}
		That kind of contract, similar to \gls{SLA} would make application developers know what guarantees they have -- or not -- on each point, even if that makes costs rise up like seen through the different auction implementations.

	\item[Lack of incentive] for regular applications. No auction mechanisms tried to segment the Fog platform into the two categories that are
		\begin{enumerate*}[(A)]
			\item edge-dependant applications that either require low latency or location-dependant features ;
			\item edge-optimized applications, eg. stream processing of \gls{IoT} events: who would want to pay more to optimize the use of the common network, while they still can process data in the Cloud ?
		\end{enumerate*}
		The latter point is the one causing troubles: why would one process (\gls{IoT}, etc.) events in the Fog while it could cost less to process them in the Cloud? \footnote{This leans towards the tragedy of the common good problem, where shared resources are over-utilized because no incentives constrain the users on the short term.} In the short term, additional development costs would be needed to enable and application to profit from the Fog. Additional costs include the cost of the actual execution on nodes. Would that cost be lower than the Cloud? What if it is not? In that case, what incentivize developers not to flood Internet links to process their data in the Cloud? Those problems can be addressed by introducing a framework to seamlessly develop \gls{FaaS} applications and integrate them in whatever the user chooses (Fog, \gls{MEC}, Cloud) \footnote{This framework would have efficient ways to deal with application metrics, logs, and data-based intelligence creation.}. About execution costs, the easiest solution would be for them to be less in Fog than in the Cloud. This latter point might require further investigations not in the scope of this report.
	...
	\item[Smarter auction-based placement mechanisms] that would factorize all complexity of resource availability, demand and interests in a collaborative dynamic. Auction mechanisms have been proposed, especially in radio spectrum usages like \citeposs{milgrom_redesigning_2017} proposal about 5G to enable some user with high interests to reserve a channel while letting other users the remaining for less valuable usages. The authors proposed a depreciating licensing model to do that. Dynamic enough for the reserved channel's owner to change is its value was less than its competitor's, thus driving innovation in the particular case of the radio band usages. Truthfulness is guaranteed because the channel is only rented, meaning the user pays the value he gives to the band. Maybe such a model could be introduced in the Fog to allow nodes to have priority functions while letting other use cases take opportunistic advantages.

	\item[Technically adapted auction mechanisms] that would not fail to exploit the ``cold-start'' phenomenon: already provisioned applications are not yet cached nor check-pointed \cite{karhula_checkpointing_2019}. Potential resides in later executions that would be accomplished at a lower cost, both in time and resource -- maybe less resources are needed for the business execution than for preparing the whole code base. This mitigates the placement problem as caching platforms could play a prominent role executing code they can store, especially when they are located at network ``cross-roads''. Thus, could functions gain not to be executed one hop away from the event source, even in latency sensitive scenarios?

	\item[Function travel] or the art of considering where the function originates. Functions can be uploaded either by an end-device, a central Cloud-based registry or a Fog based, decentralized, one. Another linked problem is about versioning of such distributed/decentralized architectures. How to update functions while avoiding downtimes and conflicts (this problem may get harder when considering part of the application executes in the Cloud).

	\item[Security and privacy] as top priority guarantees. Security of both hardware and software is required, both to trust the node and the function executing on that node, making sure it doesn't escape from its enclosure. Privacy is another point of interest that is especially relevant when designing auctions. A bid shouldSuch a Fog network will be especially welcomed if opened to a general secured protocol able to allow automatic caching and on-location optimal execution of private code in a sandboxed environment.
\end{description}
