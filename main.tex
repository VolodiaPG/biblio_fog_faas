...
\abstract{
	\gls{FaaS} applications could harness the distributed nature of Fog. This programming paradigm allows applications to be divided in independent units named functions. However, deciding how to place those still remain a challenge. Especially considering Fog is constituted of multiple computing-enabled nodes spanning from the Cloud to network edges like access points.\\
	In this combination of model, one can leverage the proximity of an application with its users to enable real-time use cases, reduce bandwidth usage, etc. ; however, multiple applications require doing so, and efficiently.\\
	This is why the examined state-of-the-art literature concentrates on finding a scheduling strategy, to provision the necessary resources while keeping in mind data-locality, latency, etc. \\
	This bibliographic report will concentrate on finding bleeding-edge questions about the literature, paving the way toward a MSc. internship. We will pay attention to the metrics taken into account, acknowledging the algorithms as well as making a case for auctions. \\
	We think auction -- and bidding -- paired with a good contracting method would translate the needs of an application and enable efficient exploitation of the Fog. Furthermore, it would provide an incentive towards the use of the Fog as well as considering \gls{FaaS} challenges (cold-start, function storage, input data) and make security and privacy a priority. This would fall right into Fog's promises.
}
...
\begin{description}
	...
	\item[\citet{cicconetti_decentralized_2021}] propose a decentralized platform to minimize response times while respecting short- and long-term fairness in stateless task assignations. Their platform does not extend to the Cloud and integrates by the standards of the \gls{MEC} model. An edge device is connected to an edge router that submits the stateless task request to an edge computer. The router decides autonomously where to forward the task in his known pool of compute-enabled nodes. The router proceeds to two actions. It updates the weights by measuring a rolling average of routed function response times submitted to computing nodes. Then it chooses a destination, avoiding congested paths thanks to data provided by a \gls{SDN} controller supervising the network. Three three different algorithms have been evaluated to route tasks, namely:
		...
		The authors conclude the \gls{SDN} plays a capital role in the system. Under load variations, performance is equally good as in regular distributed systems. Finally, exploration of hierarchical forwarding in a simulated test-bed with OpenWhisk offers interesting scalability perspectives.
...
\begin{description}
	\item[Lack of \gls{SLA}] that would detail all the challenges posed by a transition from Cloud to Fog, namely \cite{chiang_fog_2016, bonomi_fog_2012}:
		\begin{enumerate*}[(1)]
