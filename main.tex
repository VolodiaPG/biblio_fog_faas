...
\abstract{
	\gls{FaaS} applications can harness the particular distributed nature of Fog. However, deciding how to place the function-divided application functions is a challenge. Considering Fog is constituted of multiple nodes spanning from the Cloud to access points where edge devices are connected makes up most of the challenge.\\
	One can leverage the proximity to enable real-time applications, reduction in bandwidth usage, etc. ; however, multiple applications require to do so, and efficiently.\\
	This is why the examined state-of-the-art literature that concentrates on finding a scheduling strategy to provision the necessary resources, while keeping in mind data-locality, latency, etc. \\
	This report will concentrate on finding bleeding-edge questions about the literature, paving the way toward an internship. We will pay attention to the metrics taken into account as well as auctions. \\
	We think auction bidding, along with a good contracting method, would translate the needs of an application and enable efficient exploitation of the Fog. Furthermore, it would provide an incentive towards the use of the Fog as well as considering \gls{FaaS} challenges (cold-start, function storage, input data) and make security and privacy a priority. This would fall right into Fog's promises.
}
...
Questions about Fog and \gls{FaaS} are still opened about \cite{kjorveziroski_iot_2021,xie_when_2021}:
\begin{enumerate*}[(1)]
	\item scheduling ;
	\item deployment ;
	\item performance ;
...
Existing platforms usually focus on latency and resource management to apply their scheduling strategies as well as finding new metrics. eg. the transit of data about input, output, and the function themselves. Another void is the lack of contracts that would enable a developer to have guarantees, eg. in the case of latency-sensitive heart-attacks detection applications provisioned in the Cloud because another function processing \gls{IoT} filtering was executed instead.
...
Other identified challenges reside in \gls{FaaS} limitations. The worst known being investigated is the ``cold-start'' problem, ie. a delay caused by the instantiation of the runtime of the business code's environment. However, check-pointing methods could be employed to mitigate this problem, as well as good caching strategies to avoid useless multiplication of that delay. Another option is to switch to a different technology, such as WASM \cite{hykes_solomon_2019}.

The report is then divided as follows. \Cref{sec:background} further differentiates Fog and \gls{MEC}, discuss \gls{FaaS} and provides applications examples. \Cref{sec:platforms} present \gls{FaaS} platforms. Then \cref{sec:placement} discuss literature on function placement in the Fog. Finally, \cref{sec:curated} approaches a curated list of identified challenges from the previously examined literature.
